...
package me.rainyfall.module.player
...
import dragline.utils.item.ArmorComparator
import dragline.utils.item.ArmorPiece
...
class InvManager : Module() {
    ...
    private val swapArmorModeValue = ListValue("SwapMode", arrayOf( "WindowClick"), "WindowClick")
    private val items =
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (!InventoryUtils.CLICK_TIMER.hasTimePassed(delay) || (ka.target != null && noCombatValue.get()) || (!classProvider.isGuiInventory(mc.currentScreen) && invOpenValue.get()) || noMoveValue.get() && isMoving || thePlayer.openContainer != null && thePlayer.openContainer!!.windowId != 0) return
    ...
    fun isUseful(itemStack: IItemStack, slot: Int): Boolean {
        return try {
            ...
            if (classProvider.isItemSword(item) || classProvider.isItemTool(item)) {
                ...
                item
            ) && !me.rainyfall.utils.InventoryUtils.SKYWAR_BLOCK_BLACKLIST.contains(item!!.asItemBlock().block)) || classProvider.isItemBed(
                item

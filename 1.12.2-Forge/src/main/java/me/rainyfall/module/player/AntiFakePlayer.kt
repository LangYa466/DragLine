...
import dragline.event.EntityKilledEvent
...
class AntiFakePlayer : Module() {
    ...
    val CLIENT_NAME = DragLine.CLIENT_NAME
    ...
    private fun playerDeathAction(name: String, cd: Long) {
        try {
            bots++
            DragLine.fileManager.friendsConfig.addFriend(name)
        } catch (ex: Exception) {
            ex.printStackTrace()
            state = false
            state = true
        ...
        Thread {
            try {
                Thread.sleep(cd)
                protectedname = if (fakeNameProtectValue.get()) "§7§k$name" else "§7$name"
               DragLine.fileManager.friendsConfig.removeFriend(name)
                bots--
    ...
    private fun clearAll() {
        DragLine.fileManager.friendsConfig.clearFriends()
        bots = 0
    }

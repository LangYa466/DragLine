...
import dragline.api.minecraft.util.WMathHelper.wrapAngleTo180_float
...
@ModuleInfo(
    name = "Scaffold3",
    description = "Skid & Modify by Ryf, FuLing and ColorByte.",
...
class Scaffold3 : Module() {
    ...
    private val grimSprintValue = BoolValue("GrimSprintRotation",true)
    ...
    private val counterDisplayModeValue = ListValue("CounterDisplayMode", arrayOf("Rise", "Rise6"), "Rise6")
    ...
    fun onRender2D(event: Render2DEvent) {
        if (counterDisplayValue.get()) {
            when (counterDisplayModeValue.get().toLowerCase()) {
                "rise" -> {
                    ...
                    if (slot != -1) {
                        if (mc2.player.inventory.getCurrentItem() != null) {
                            val handItem = mc2.player.inventory.getCurrentItem().item
                            if (handItem is ItemBlock) {
                                stack = mc2.player.inventory.getCurrentItem()
                            }
                        }
                        if (stack == barrier) {
                            stack = mc2.player.inventory.getStackInSlot(InventoryUtils.findAutoBlockBlock() - 36)
                            if (stack == null) {
                                stack = barrier
                            }
                        }
                    ...
                    mc2.renderItem.renderItemIntoGUI(stack, width / 2 - 9, (height * 0.8 - 20).toInt())
                ...
                "rise6" -> {
                    ...
                    val progress = (System.currentTimeMillis() - lastMS).toFloat() / 100f
                    val scaledResolutions = ScaledResolution(mc2)
                    val eeasing = EaseUtils.apply(
                        EaseUtils.EnumEasingType.valueOf("BACK"),
                        EaseUtils.EnumEasingOrder.valueOf("In"),
                        progress.toDouble()
                    ).toFloat()
                    val info = "Amount: $blocksAmount"
                    val height = scaledResolutions.scaledHeight
                    val width = scaledResolutions.scaledWidth
                    val w2 = (mc.fontRendererObj.getStringWidth(info))
                    RenderUtils.drawRoundedCornerRect(
                        (width - w2 - 20) / 2f,
                        height * 0.8f - 8f,

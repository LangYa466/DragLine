...
@ModuleInfo(
    name = "Scaffold2",
    description = "Form Kid Fix By nelly",
...
class Scaffold2 : Module() {
    ...
    private fun onUpdate(event: UpdateEvent) {
        ...
        if (!sprintValue.get()) {
            ...
            val sprintMode = sprintModeValue.get().toLowerCase()

            when (sprintMode) {
                "off" -> mc.thePlayer!!.sprinting = false
                "ground" -> if(!mc.thePlayer!!.onGround) {
                    mc.thePlayer!!.sprinting = false
                } else {
                    mc.thePlayer!!.sprinting = true
                }
                "air" -> if (!mc.thePlayer!!.isAirBorne)  {
                    mc.thePlayer!!.sprinting = false
                } else {
                    mc.thePlayer!!.sprinting = true
                }

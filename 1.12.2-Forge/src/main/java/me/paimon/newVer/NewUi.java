...
import me.paimon.newVer.element.CategoryElement;
import me.paimon.newVer.element.SearchElement;
import me.paimon.newVer.element.module.ModuleElement;
import me.paimon.ui.NewGUI;
import me.paimon.utils.ClickEffect;
import me.paimon.utils.IconManager;
...
public class NewUi extends GuiScreen {
    ...
    private void drawFullSized(int mouseX, int mouseY, float partialTicks, Color accentColor) {
        RenderUtils.drawRoundedRect2(30F, 30F, this.width - 30F, this.height - 30F, 8F, new Color(238, 238, 238).getRGB());
        ...
        if (mc.player != null) {
            ...
            if(!got){
                try {
                    mc.getTextureManager().loadTexture(
                            ...
                            (AbstractTexture) new DynamicTexture(ImageIO.read(new URL("https://q.qlogo.cn/headimg_dl?dst_uin=3054086606&spec=640&img_type=png"))));
                    got = true;
            ...
            RenderUtils.drawImage(DragLine.wrapper.getClassProvider().createResourceLocation("sb"), 0, 0, 50, 50);
            glDepthMask(true);
            glDisable(GL_BLEND);
        ...
        RenderUtils.drawRoundedRect2(32F, startYAnim, 34F, endYAnim, 1F, accentColor.getRGB());

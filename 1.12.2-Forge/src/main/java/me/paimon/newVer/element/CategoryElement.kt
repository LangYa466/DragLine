...
class CategoryElement(val category: ModuleCategory): MinecraftInstance() {
    ...
    init {
        DragLine.moduleManager.modules.filter { it.category == category }.forEach { moduleElements.add(
            ModuleElement(it)
        ) }
    }
    ...
    fun drawLabel(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, height: Float) {
        if (focused)
            RenderUtils.drawRoundedRect2(x + 3F, y + 3F, x + width - 3F, y + height - 3F, 3F, ColorManager.dropDown.rgb)
        else if (MouseUtils.mouseWithinBounds(mouseX, mouseY, x, y, x + width, y + height))
            RenderUtils.drawRoundedRect2(x + 3F, y + 3F, x + width - 3F, y + height - 3F, 3F, Color(241, 243, 247).rgb)
    ...
    fun drawPanel(mX: Int, mY: Int, x: Float, y: Float, width: Float, height: Float, wheel: Int, accentColor: Color) {
        ...
        Fonts.font20.drawString("$name", x - 190F, y - 12F, Color(26, 26, 26).getRGB())
    ...
    private fun drawScroll(x: Float, y: Float, width: Float, height: Float) {
        if (lastHeight > height - 60F) {
            ...
            RenderUtils.drawRoundedRect2(x + width - 6F, y + 5F + multiply, x + width - 4F, y + 5F + (height - 60F) * ((height - 60F) / lastHeight) + multiply, 1F, 0x50FFFFFF)
        }
    }

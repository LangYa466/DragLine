package me.paimon.newVer.element.module

import dragline.DragLine
import me.paimon.newVer.ColorManager
import me.paimon.newVer.MouseUtils
import me.paimon.newVer.element.components.ToggleSwitch
import me.paimon.newVer.element.module.value.ValueElement
import me.paimon.newVer.element.module.value.impl.BooleanElement
import me.paimon.newVer.element.module.value.impl.FloatElement
import me.paimon.newVer.element.module.value.impl.IntElement
import me.paimon.newVer.element.module.value.impl.ListElement
...
import dragline.utils.render.Stencil
...
class ModuleElement(val module: Module): MinecraftInstance() {
    ...
    companion object {
        protected val expandIcon = DragLine.wrapper.classProvider.createResourceLocation("liquidbounce/clickgui/expand.png") }
    ...
    fun drawElement(mouseX: Int, mouseY: Int, x: Float, y: Float, width: Float, height: Float, accentColor: Color): Float {
        animPercent = animPercent.animSmooth(if (expanded) 100F else 0F, 0.5F) as Float
        ...
        RenderUtils.drawRoundedRect2(x + 9.5F, y + 4.5F, x + width - 9.5F, y + height + animHeight - 4.5F, 4F, ColorManager.buttonOutline.rgb)
        Stencil.write(true)
        RenderUtils.drawRoundedRect2(x + 10F, y + 5F, x + width - 10F, y + height + animHeight - 5F, 4F, ColorManager.moduleBackground.rgb)
        Stencil.erase(true)
//        RenderUtils.drawRect(x + 10F, y + height - 5F, x + width - 10F, y + height - 4.5F, 4281348144L.toInt())

        Fonts.font40.drawString(module.name, x + 20F, y + height / 2F - Fonts.font40.fontHeight + 3F, Color(26, 26, 26).getRGB())
        ...
        RenderUtils.drawRoundedRect2(
                x + 25F + Fonts.font40.getStringWidth(module.name),
                y + height / 2F - Fonts.font40.fontHeight + 1F,
                x + 35F + Fonts.font40.getStringWidth(module.name) +Fonts.font30.getStringWidth(keyName),
                y + height / 2F+4f, 2F, BlendUtils.blend(Color(4282729797L.toInt()), Color(219, 219, 219), fadeKeybind.toDouble()).rgb)
       Fonts.font30.drawString(keyName, x + 30.5F + Fonts.font40.getStringWidth(module.name), y + height / 2F - Fonts.font40.fontHeight + 5.5F, Color(26, 26, 26).getRGB())

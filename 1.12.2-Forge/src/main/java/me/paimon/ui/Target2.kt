...
import me.paimon.utils.ShapeType
...
class Target2 : Element(-46.0,-40.0,1F,Side(Side.Horizontal.MIDDLE,Side.Vertical.MIDDLE)) {
    ...
    private fun drawTenacity5(target: IEntityLivingBase,easingHealth: Float) {
        ...
        // text
        Fonts.font40.drawCenteredString(target.name!!, 47 + (additionalWidth / 2f), 1f + Fonts.font40.fontHeight, Color.WHITE.rgb, false)
        ...
        Fonts.font40.drawString(infoStr, 47f + ((additionalWidth - Fonts.font40.getStringWidth(infoStr)) / 2f), 45f - (Fonts.font40.fontHeight), Color.WHITE.rgb, false)
    ...
    private fun drawRice(entity: IEntityLivingBase,easingHealth: Float) {
            ...
            if (gotDamaged) {
                for (j in 0..(generateAmountValue.get())) {
                    ...
                    val color2 = RenderUtils.getGradientOffset(
                        ...
                        Color(HUD.redValue2.get(), HUD.greenValue2.get(), HUD.blueValue2.get()),
                        10.0
                    )
                    particleList.add(
                        Particle(
        ...
        Stencil.erase(true)
        RenderUtils.drawRoundedRect(5F, 42F, length - maxHealthLength, 48F, 3,Color(255,43,24).rgb)
        Stencil.dispose()
...
//        GL11.glTranslated(renderX, renderY, 0.0)
        ...
        GlStateManager.resetColor()
        Fonts.font30.drawString(target.name!!,34f,20f-Fonts.font30.fontHeight,Color(255,255,255).rgb)
    ...
    private fun drawnewnovo(target: IEntityLivingBase, easingHealth: Float){
        ...
        RenderUtils.drawGradientSideways(
            ...
            24.0, Palette.fade2(customColor, counter1[0],Fonts.font35.fontHeight).rgb,
            Palette.fade2(customColor1, counter2[0], Fonts.font35.fontHeight).rgb
        )
        //easingHealth += ((target.health - easingHealth) / 2.0F.pow(10.0F - fadeSpeed.get())) * RenderUtils.deltaTime
    ...
    private fun getTBorder():Border?{
        return when(modeValue.get().toLowerCase()){
            ...
            "tenacity5" -> Border(-2F, 3F, 62F + mc.thePlayer!!.name.toString().let(Fonts.font40::getStringWidth).coerceAtLeast(75).toFloat(), 50F)
            "romantic" -> Border(0F, 0F, 120F, 36F)

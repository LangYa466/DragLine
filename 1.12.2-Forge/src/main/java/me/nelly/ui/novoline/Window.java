...
public class Window {
    ...
    public final ArrayList<me.nelly.ui.novoline.Button> buttons = Lists.newArrayList();
    public boolean drag;
    ...
    public Window(ModuleCategory category, int x, int y) {
        ...
        max = 120;
        int y2 = y + 22;
        for (Module c : DragLine.moduleManager.getModules()) {
            if (c.getCategory() != category)
                continue;
            buttons.add(new me.nelly.ui.novoline.Button(c, x + 5, y2));
            y2 += 15;//15
        }
        for (me.nelly.ui.novoline.Button b2 : buttons) {
            b2.setParent(this);
        }
    }
    int wheely;
    ...
    public void render(int mouseX, int mouseY) {
        ...
        int iY = y + 22;
        totalY = 17;
        for (me.nelly.ui.novoline.Button b3 : buttons) {
       ...
       // final GameFontRenderer novoicons = Fonts.fontNovoIcon20;
        final IFontRenderer novoicons = Fonts.font35;
        if (category.name().equals("Combat")) {

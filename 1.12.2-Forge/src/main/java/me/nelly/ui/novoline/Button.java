...
public class Button {
    ...
    public void render(int mouseX, int mouseY, int x11,int y11,int x22,int y22) {
        ...
        //GL11.glEnable(3089);
        //limitation.cut();
        hover = mouseX > x - 7 && mouseX < x + 85 && mouseY > y - 6 && mouseY < y + font.getFontHeight() + 4;
        RenderUtils.drawRect(x - 5, y - 5, x + 85 + parent.allX, y + font.getFontHeight() + 5, new Color(40, 40, 40).getRGB());
        RenderUtils.drawRect(x - 5, y - 5 - 1, x + 85 + parent.allX, y + font.getFontHeight() + 3 + 1,
                hudcolorwithalpha());//Button Font List
        rticks++;

        Color Ranbow = new Color(Color.HSBtoRGB(
                (float) ((double) Minecraft.getMinecraft().player.ticksExisted / 50.0 + Math.sin((double) 0 / 50.0 * 1.6))
                        % 1.0f,
                0.5f, 1.0f));
        ...
        font.drawStringWithShadow(cheat.getName(), x, (int) (y - 1), -1);
        if (expand) {
    ...
    private int hudcolorwithalpha() {
        ...
        Color Ranbow = new Color(Color.HSBtoRGB(
                (float) ((double) Minecraft.getMinecraft().player.ticksExisted / 50.0 + Math.sin((double) 50 / 50.0 * 1.6))
                        % 1.0f,
                0.5f, 1.0f));
    ...
    public void click(int mouseX, int mouseY, int button) {
        ...
        if (mouseX > x - 7 && mouseX < x + 85 + parent.allX && mouseY > y - 6 && mouseY < y + Fonts.font35.getFontHeight()) {
            if (button == 0) {

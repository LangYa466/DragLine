...
import dragline.features.module.modules.combat.FastBow;
...
public final class RotationUtils extends MinecraftInstance implements Listenable {
    ...
    public static VecRotation searchCenterN(final IAxisAlignedBB bb, final boolean outborder, final boolean random,
    ...
    public static void faceBow(final IEntity target, final boolean silent, final boolean predict, final float predictSize) {
        ...
        float velocity = DragLine.moduleManager.getModule(FastBow.class).getState() ? 1F : player.getItemInUseDuration() / 20F;

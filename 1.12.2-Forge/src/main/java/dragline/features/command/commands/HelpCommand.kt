...
class HelpCommand : Command("help") {
    ...
    override fun execute(args: Array<String>) {
        ...
        val maxPageDouble = DragLine.commandManager.commands.size / 8.0
        ...
        val commands = DragLine.commandManager.commands.sortedBy { it.command }

        var i = 8 * (page - 1)
        while (i < 8 * page && i < commands.size) {
            val command = commands[i]

            ClientUtils.displayChatMessage("§6> §7${DragLine.commandManager.prefix}${command.command}${if (command.alias.isEmpty()) "" else " §7(§8" + Strings.join(command.alias, "§7, §8") + "§7)"}")
            i++
        }

        ClientUtils.displayChatMessage("§a------------\n§7> §c${DragLine.commandManager.prefix}help §8<§7§lpage§8>")
    }
}
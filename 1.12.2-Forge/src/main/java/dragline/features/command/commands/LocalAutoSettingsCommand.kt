...
import dragline.utils.SettingsUtils
...
class LocalAutoSettingsCommand : Command("localautosettings", "localsetting", "localsettings", "localconfig") {
        ...
        if (args.size > 1) {
            when {
                args[1].equals("load", ignoreCase = true) -> {
                    if (args.size > 2) {
                        val scriptFile = File(DragLine.fileManager.settingsDir, args[2])
    ...
    private fun getLocalSettings(): Array<File>? = DragLine.fileManager.settingsDir.listFiles()
}
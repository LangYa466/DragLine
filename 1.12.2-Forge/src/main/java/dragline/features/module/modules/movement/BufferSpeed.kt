...
class BufferSpeed : Module() {
    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        if (DragLine.moduleManager.getModule(Speed::class.java)!!.state || noHurtValue.get() && thePlayer.hurtTime > 0) {
            reset()
            return
        }
        ...
        if (thePlayer.onGround) {
            ...
            if (iceValue.get() && (getBlock(blockPos.down()) == classProvider.getBlockEnum(BlockType.ICE) || getBlock(blockPos.down()) == classProvider.getBlockEnum(
                    BlockType.ICE_PACKED))) {
            ...
            if (wallValue.get()) {
                when (wallModeValue.get().toLowerCase()) {
                    "old" -> if (thePlayer.isCollidedVertically && isNearBlock || !classProvider.isBlockAir(getBlock(
                            WBlockPos(thePlayer.posX, thePlayer.posY + 2.0, thePlayer.posZ)
                        ))) {
        ...
        get() {
            ...
            for (blockPos in blocks) {
                ...
                if ((collisionBoundingBox == null || collisionBoundingBox.maxX ==
                        ...
                        !blockState.block.isTranslucent(blockState) && blockState.block == classProvider.getBlockEnum(
                        BlockType.WATER) &&
                        !classProvider.isBlockSlab(blockState.block) || blockState.block == classProvider.getBlockEnum(
                        BlockType.BARRIER)) return true
            }
            return false
        }
}
...
import dragline.api.minecraft.util.WVec3i
...
import dragline.utils.VecRotation
...
class NoFall : Module() {
    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        if (!state || DragLine.moduleManager.getModule(FreeCam::class.java)!!.state)
            return
        ...
        if (modeValue.get().equals("MLG", ignoreCase = true)){
            if (event.eventState == EventState.PRE) {
                ...
                if (mc.thePlayer!!.fallDistance > minFallDistance.get()) {
                    ...
                    var ok: Boolean = WVec3(mc.thePlayer!!.posX, mc.thePlayer!!.posY + mc.thePlayer!!.eyeHeight, mc.thePlayer!!.posZ).distanceTo(
                        WVec3(collision.pos).addVector(0.5, 0.5, 0.5)) < mc.playerController.blockReachDistance + sqrt(0.75)
                    ...
                    for (i in 36..44) {
                        ...
                        if (itemStack != null && (itemStack.item == classProvider.getItemEnum(ItemType.WATER_BUCKET) || classProvider.isItemBlock(itemStack.item) && (itemStack.item?.asItemBlock())?.block == classProvider.getBlockEnum(
                                BlockType.WEB))) {
                            index = i - 36
    ...
    fun getJumpEffect(): Int {
        return if (mc.thePlayer!!.isPotionActive(classProvider.getPotionEnum(PotionType.JUMP))) mc.thePlayer!!.getActivePotionEffect(classProvider.getPotionEnum(
            PotionType.JUMP))!!.amplifier + 1 else 0
    }

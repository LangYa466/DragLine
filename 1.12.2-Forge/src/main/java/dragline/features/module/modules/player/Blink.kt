...
import dragline.features.module.modules.render.Breadcrumbs
...
class Blink : Module() {
    ...
    private val cancelc0fValue = BoolValue("CancelC0f", false)
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if(event.packet.unwrap() is CPacketConfirmTransaction && cancelc0fValue.get()) {
            event.cancelEvent()
        }
    }
    ...
    fun onRender3D(event: Render3DEvent?) {
        val breadcrumbs = DragLine.moduleManager.getModule(Breadcrumbs::class.java) as Breadcrumbs?

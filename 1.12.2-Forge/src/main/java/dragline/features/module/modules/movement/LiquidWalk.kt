...
class LiquidWalk : Module() {
    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        when (modeValue.get().toLowerCase()) {
            "ncp", "vanilla" -> if (collideBlock(thePlayer.entityBoundingBox, classProvider::isBlockLiquid) && thePlayer.isInsideOfMaterial(classProvider.getMaterialEnum(
                    MaterialType.AIR)) && !thePlayer.sneaking) thePlayer.motionY = 0.08
            "aac" -> {
            ...
            "aac3.3.11" -> if (thePlayer.isInWater) {
                ...
                if (thePlayer.isCollidedHorizontally) thePlayer.motionY = 0.24 else if (mc.theWorld!!.getBlockState(
                        WBlockPos(thePlayer.posX, thePlayer.posY + 1.0, thePlayer.posZ)
                    ).block != classProvider.getBlockEnum(BlockType.AIR)) thePlayer.motionY += 0.04
            }

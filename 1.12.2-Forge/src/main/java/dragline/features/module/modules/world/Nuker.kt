...
import dragline.utils.block.BlockUtils.searchBlocks
...
class Nuker : Module() {
    ...
    fun onUpdate(event: UpdateEvent) {
        // Block hit delay
        if (blockHitDelay > 0 && !DragLine.moduleManager[FastBreak::class.java]!!.state) {
            blockHitDelay--
            return
        }
        ...
        if (!mc.playerController.isInCreativeMode) {
            ...
            do{
                ...
                // Call auto tool
                val autoTool = DragLine.moduleManager.getModule(AutoTool::class.java) as AutoTool
                if (autoTool.state)
                ...
                if (currentDamage >= 1F) {
                    mc.netHandler.addToSendQueue(classProvider.createCPacketPlayerDigging(
                        ICPacketPlayerDigging.WAction.STOP_DESTROY_BLOCK, blockPos, classProvider.getEnumFacing(
                            EnumFacingType.DOWN)))
                    mc.playerController.onPlayerDestroyBlock(blockPos, classProvider.getEnumFacing(EnumFacingType.DOWN))
                    blockHitDelay = hitDelayValue.get()
                    currentDamage = 0F

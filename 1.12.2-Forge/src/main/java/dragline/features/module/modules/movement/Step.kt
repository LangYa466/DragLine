...
import dragline.features.module.modules.exploit.Phase
...
class Step : Module() {
    ...
    fun onStep(event: StepEvent) {
        ...
        if (DragLine.moduleManager[Phase::class.java]!!.state) {
            event.stepHeight = 0F
            return
        }

        // Some fly modes should disable step
        val fly = DragLine.moduleManager[Fly::class.java] as Fly
        if (fly.state) {

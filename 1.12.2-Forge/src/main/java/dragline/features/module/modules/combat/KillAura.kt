...
import dragline.features.module.modules.render.FreeCam
import dragline.features.module.modules.client.HUD
import dragline.features.module.modules.world.Scaffold
import dragline.ui.font.GameFontRenderer.Companion.getColorIndex
import dragline.utils.extensions.getDistanceToEntityBox
import dragline.utils.extensions.isAnimal
import dragline.utils.extensions.isClientFriend
import dragline.utils.extensions.isMob
...
class KillAura : Module() {
    ...
    // CPS - Attack speed
    val maxCPS: IntegerValue = object : IntegerValue("MaxCPS", 8, 1, 30) {
    ...
    val minCPS: IntegerValue = object : IntegerValue("MinCPS", 5, 1, 30) {
        override fun onChanged(oldValue: Int, newValue: Int) {
            val i = maxCPS.get()
            if (i < newValue) set(i)
    ...
    private val rotations = ListValue(
        "RotationMode",
        arrayOf("None", "Liquidbounce", "BackTrack", "HytRotation","GrimAC"),
        "HytRotation"
    )
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (hyt180fovfixValue.get() && target != null) {
            if (RotationUtils.getRotationDifference(target) > 66.0) {
        ...
        (DragLine.moduleManager[StrafeFix::class.java] as StrafeFix).applyForceStrafe(
        ...
        if (target != null && currentTarget != null && mc.thePlayer!!.getCooledAttackStrength(
                0.0F
            ) >= cooldownValue.get()
        ) {
            while (clicks > 0) {
    ...
    fun onRender3D(event: Render3DEvent) {
        ...
        fun getColor(ent: IEntityLivingBase?): Color? {
            ...
            if (ent is EntityLivingBase) {
                ...
                if (colorModeValue.get().equals("Health", ignoreCase = true)) return ColorUtils.getHealthColor(
                    ent.health,
                    ent.maxHealth
                )
                if (colorTeam.get()) {
            ...
            return when (colorModeValue.get()) {
                "Gident" -> RenderUtils.getGradientOffset(
                    ...
                    Color(HUD.redValue2.get(), HUD.greenValue2.get(), HUD.blueValue2.get()), (Math.abs(
                        System.currentTimeMillis() / HUD.gidentspeed.get().toDouble()
                    ))
                )
                ...
                "Rainbow" -> ColorUtils.hslRainbow(
                    counter[0] * 100 + 1,
                    indexOffset = 100 * HUD.rainbowSpeed.get()
                )
    ...
    private fun isEnemy(entity: IEntity?): Boolean {
        if (entity != null && classProvider.isEntityLivingBase(entity) && (EntityUtils.targetDead || isAlive(entity.asEntityLivingBase())) && entity != mc.thePlayer) {
            ...
            if (EntityUtils.targetPlayer && classProvider.isEntityPlayer(entity)) {
                ...
                if (player.isClientFriend() && !DragLine.moduleManager[NoFriends::class.java].state) {
                    return false
                }

                val teams = DragLine.moduleManager.getModule(Teams::class.java) as Teams
    ...
    private fun attackEntity(entity: IEntityLivingBase) {
        ...
        // Call attack event
        DragLine.eventManager.callEvent(AttackEvent(entity))
        ...
        val criticals = DragLine.moduleManager[Criticals::class.java] as Criticals
    ...
    private fun updateRotations(entity: IEntity): Boolean {
        ...
        if (rotations.get().equals("grimac", ignoreCase = true)) {
            ...
            val distance = if (mc.thePlayer!!.getDistanceToEntityBox(entity) > rangeValue.get()) {
                maxRange
            } else {
                rangeValue.get()
            }
            val (_, rotation) = RotationUtils.searchCenterN(
                boundingBox,
                outborderValue.get() && !attackTimer.hasTimePassed(attackDelay / 2),
                randomCenterValue.get(),
                predictValue.get(),
    ...
    private fun updateHitable() {
        ...
        if (raycastValue.get()) {
            ...
            if (raycastValue.get() && raycastedEntity != null && classProvider.isEntityLivingBase(raycastedEntity)
                && (DragLine.moduleManager[NoFriends::class.java].state || !(classProvider.isEntityPlayer(
                    raycastedEntity
    ...
    private fun startBlocking(interactEntity: IEntity, interact: Boolean) {
        ...
        if (!autoBlockValue.get().equals("Fake", true)) {
            KillAura.fakeab = false
            return
        }
    ...
    private val cancelRun: Boolean
        inline get() = mc.thePlayer!!.spectator || !isAlive(mc.thePlayer!!)
                || DragLine.moduleManager[Blink::class.java].state || DragLine.moduleManager[FreeCam::class.java].state || (noScaffoldValue.get() && (DragLine.moduleManager[Scaffold::class.java].state))

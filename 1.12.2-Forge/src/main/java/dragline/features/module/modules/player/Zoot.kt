...
class Zoot : Module() {
    ...
    private fun hasBadEffect(): Boolean {
        ...
        return thePlayer.isPotionActive(classProvider.getPotionEnum(PotionType.HUNGER)) || thePlayer.isPotionActive(classProvider.getPotionEnum(
            PotionType.MOVE_SLOWDOWN)) ||
                thePlayer.isPotionActive(classProvider.getPotionEnum(PotionType.DIG_SLOWDOWN)) || thePlayer.isPotionActive(classProvider.getPotionEnum(
            PotionType.HARM)) ||
                thePlayer.isPotionActive(classProvider.getPotionEnum(PotionType.CONFUSION)) || thePlayer.isPotionActive(classProvider.getPotionEnum(
            PotionType.BLINDNESS)) ||
                thePlayer.isPotionActive(classProvider.getPotionEnum(PotionType.WEAKNESS)) || thePlayer.isPotionActive(classProvider.getPotionEnum(
            PotionType.WITHER)) || thePlayer.isPotionActive(classProvider.getPotionEnum(PotionType.POISON))
    }

 ...
 */
package dragline.ui.client.altmanager.sub;
...
import dragline.utils.TabUtils;
...
public class GuiAdd extends WrappedGuiScreen {
    ...
    public void actionPerformed(IGuiButton button) throws IOException {
        ...
        switch (button.getId()) {
            case 0:
                ...
                break;
            case 1:
                if (DragLine.fileManager.accountsConfig.accountExists(username.getText())) {
    ...
    private void addAccount(final String name, final String password) {
        if (DragLine.fileManager.accountsConfig.accountExists(name)) {
            status = "§cThe account has already been added.";
            return;
        }
        ...
        new Thread(() -> {
            ...
            DragLine.fileManager.accountsConfig.getAccounts().add(account);

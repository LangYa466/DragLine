 ...
 */
package dragline.ui.client.altmanager;
...
import dragline.ui.client.altmanager.sub.*;
import dragline.api.minecraft.client.gui.IGuiScreen;
import dragline.api.minecraft.client.gui.IGuiTextField;
import dragline.api.util.WrappedGuiScreen;
import dragline.api.util.WrappedGuiSlot;
import dragline.ui.client.altmanager.sub.*;
import dragline.ui.client.altmanager.sub.altgenerator.GuiMCLeaks;
...
import dragline.utils.login.LoginUtils;
...
public class GuiAltManager extends WrappedGuiScreen {
    ...
    public static void loadGenerators() {
        try {
            ...
            final JsonElement jsonElement = new JsonParser().parse(HttpUtils.get(DragLine.CLIENT_CLOUD + "/generators.json"));
    ...
    public static String login(final MinecraftAccount minecraftAccount) {
        ...
        if (result == LoginUtils.LoginResult.LOGGED) {
            ...
            DragLine.fileManager.saveConfig(DragLine.fileManager.accountsConfig);
    ...
    public void initGui() {
        ...
        for (int i = 0; i < DragLine.fileManager.accountsConfig.getAccounts().size(); i++) {
            MinecraftAccount minecraftAccount = DragLine.fileManager.accountsConfig.getAccounts().get(i);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Fonts.font35.drawCenteredString(this.searchField.getText().isEmpty() ? (DragLine.fileManager.accountsConfig.getAccounts().size() + " Alts") : this.altsList.accounts.size() + " Search Results", representedScreen.getWidth() / 2.0f, 18, 0xffffff);
        ...
        switch (button.getId()) {
            ...
            case 2:
                if (altsList.getSelectedSlot() != -1 && altsList.getSelectedSlot() < altsList.getSize()) {
                    DragLine.fileManager.accountsConfig.removeAccount(this.altsList.accounts.get(altsList.getSelectedSlot()));
                    DragLine.fileManager.saveConfig(DragLine.fileManager.accountsConfig);
            ...
            case 7:
                ...
                while ((line = bufferedReader.readLine()) != null) {
                    ...
                    if (!DragLine.fileManager.accountsConfig.accountExists(accountData[0])) {
                        if (accountData.length > 1)
                            DragLine.fileManager.accountsConfig.addAccount(accountData[0], accountData[1]);
                        else
                            DragLine.fileManager.accountsConfig.addAccount(accountData[0]);
                    }
                }
            ...
            case 88:
                ...
                break;
            case 9:
                mc.displayGuiScreen(classProvider.wrapGuiScreen(new GuiTheAltening()));
                break;
            case 10:
            ...
            case 11:
                ...
                break;
            case 12:
                if (DragLine.fileManager.accountsConfig.getAccounts().size() == 0) {
                    status = "§cThe list is empty.";
                    return;
                }
                ...
                try {
                    ...
                    for (MinecraftAccount account : DragLine.fileManager.accountsConfig.getAccounts()) {
    ...
    private class GuiList extends WrappedGuiSlot {
        ...
        private void updateAccounts(String search) {
            if (search == null || search.isEmpty()) {
                this.accounts = DragLine.fileManager.accountsConfig.getAccounts();
                return;
            }

 ...
 */
@ElementInfo(name = "ChineseArrayList", single = true)
class Arraylist2(
    ...
    override fun drawElement(): Border? {
        val fontRenderer = FontLoaders.CUSTOM14
                            ...
                            colorMode.equals("Gradinet", true) -> RenderUtils.getGradientOffset(
                                ...
                                (Math.abs(
                                    System.currentTimeMillis() / gidentspeed.get()
                                        .toDouble() + (module.higt / fontRenderer.height)
                                ) / 10)
                            ).rgb
    ...
    override fun updateElement() {
        modules = if (abcOrder.get()) DragLine.moduleManager.modules
            ...
            .filter { it.array && (if (hAnimation.get().equals("none", ignoreCase = true)) it.state else it.slide > 0) }
            .sortedBy { -FontLoaders.GenShin.getStringWidth(getModName(it)) }
        sortedModules = if (abcOrder.get()) DragLine.moduleManager.modules.toList()
        else DragLine.moduleManager.modules.sortedBy { -FontLoaders.GenShin.getStringWidth(getModName(it)) }.toList()
    }

...
class ScoreboardElement(x: Double = 5.0, y: Double = 0.0, scale: Float = 1F,
    ...
    override fun drawElement(): Border? {
        ...
        val worldScoreboard: Scoreboard = mc2.world.scoreboard
        var currObjective: ScoreObjective? = null
        val playerTeam = worldScoreboard.getPlayersTeam(mc2.player.name)

        if (playerTeam != null) {
            currObjective = worldScoreboard.getObjectiveInDisplaySlot(3)
        }
        ...
        for (score in scoreCollection) {
            ...
            val width = "${ScorePlayerTeam.formatPlayerName(scorePlayerTeam, score.playerName)}: ${WEnumChatFormatting.RED}${score.scorePoints}"
        ...
        // draw main rect?
        RoundedUtil.drawRound(l1 - 7f, -5f, 9f, maxHeight + fontRenderer.fontHeight + 5f, radiusvalue.get().toFloat(),blurValue.get(),Color(0,0,0,backgroundColorAlphaValue.get()))

        if(shadowvalue.get()) {
            RenderUtils.drawShadow(l1 - 7f, -5f, -l1 + 16f, maxHeight + fontRenderer.fontHeight + 10f, 255f)
        }
        ...
        scoreCollection.forEachIndexed { index, score ->
            ...
            if (rectValue.get()) {
                ...
                RenderUtils.drawRect(2F, if (index == scoreCollection.size - 1) -2F else height.toFloat(), 5F, if (index == 0) fontRenderer.fontHeight.toFloat() else height.toFloat() + fontRenderer.fontHeight * 2F, rectColor)
            }
        }

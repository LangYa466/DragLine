...
class TabGUI(x: Double = 5.0, y: Double = 25.0) : Element(x = x, y = y) {
    ...
    init {
        for (category in ModuleCategory.values()) {
            ...
            DragLine.moduleManager.modules
    ...
    override fun drawElement(): Border? {
        ...
        RoundedUtil.drawRound(1F, 0F, width.get(), guiHeight, 0f,true,backgroundColor)

        if(shadow.get()) {
            RenderUtils.drawShadow(1F, 0F, width.get(), guiHeight, 255.0F)
        }

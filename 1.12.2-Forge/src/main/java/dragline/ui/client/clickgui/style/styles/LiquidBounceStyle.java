 ...
 */
package dragline.ui.client.clickgui.style.styles;
...
public class LiquidBounceStyle extends Style {
    ...
    public void drawPanel(int mouseX, int mouseY, Panel panel) {
        ...
        if(panel.getScrollbar() && panel.getFade() > 0) {
            ...
            RenderUtils.drawRect(panel.getX() - 2, panel.getY() + 30 + (panel.getFade() - 24F) / (panel.getElements().size() - ((ClickGUI) DragLine.moduleManager.getModule(ClickGUI.class)).maxElementsValue.get()) * panel.getDragged() - 10.0f, panel.getX(), panel.getY() + 40 + (panel.getFade() - 24.0f) / (panel.getElements().size() - ((ClickGUI) DragLine.moduleManager.getModule(ClickGUI.class)).maxElementsValue.get()) * panel.getDragged(), Integer.MIN_VALUE);
        }
    }

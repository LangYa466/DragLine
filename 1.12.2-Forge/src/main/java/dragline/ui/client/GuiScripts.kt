...
class GuiScripts(private val prevGui: IGuiScreen) : WrappedGuiScreen() {
        ...
        when (button.id) {
            ...
            1 -> try {
                ...
                if (fileName.endsWith(".js")) {
                    DragLine.scriptManager.importScript(file)
                    ...
                    while (entries.hasMoreElements()) {
                        ...
                        val entryFile = File(DragLine.scriptManager.scriptsFolder, entryName)
                    ...
                    scriptFiles.forEach { scriptFile -> DragLine.scriptManager.loadScript(scriptFile) }
            ...
            2 -> try {
                if (list.getSelectedSlot() != -1) {
                    val script = DragLine.scriptManager.scripts[list.getSelectedSlot()]

                    DragLine.scriptManager.deleteScript(script)
            ...
            3 -> try {
                DragLine.scriptManager.reloadScripts()
            } catch (t: Throwable) {
                ClientUtils.getLogger().error("Something went wrong while reloading all scripts.", t)
                MiscUtils.showErrorPopup(t.javaClass.name, t.message)
            }
            4 -> try {
                Desktop.getDesktop().open(DragLine.scriptManager.scriptsFolder)
            } catch (t: Throwable) {
    ...
    private inner class GuiList(gui: IGuiScreen) :
        ...
        internal fun getSelectedSlot() = if (selectedSlot > DragLine.scriptManager.scripts.size) -1 else selectedSlot

        override fun getSize() = DragLine.scriptManager.scripts.size
        ...
        override fun drawSlot(id: Int, x: Int, y: Int, var4: Int, var5: Int, var6: Int) {
            val script = DragLine.scriptManager.scripts[id]

    ...
    override fun actionPerformed(button: IGuiButton) {
        when (button.id) {
            ...
            3 -> {
                ...
                try {
                    Files.copy(file.toPath(), FileOutputStream(DragLine.fileManager.backgroundFile))

                    val image = ImageIO.read(FileInputStream(DragLine.fileManager.backgroundFile))
                    val location = classProvider.createResourceLocation("liquidbounce/background.png")

                    DragLine.background = location
                    ...
                    DragLine.fileManager.backgroundFile.delete()
                }
            }
            4 -> {
                DragLine.background = null
                DragLine.fileManager.backgroundFile.delete()
            }

...
import dragline.features.command.shortcuts.Shortcut
...
class ShortcutsConfig(file: File) : FileConfig(file) {
    ...
    override fun loadConfig() {
        ...
        for (shortcutJson in jsonElement) {
            ...
            for (scriptCommand in scriptJson) {
                ...
                val command = DragLine.commandManager.getCommand(commandName) ?: continue

                script.add(Pair(command, arguments.map { it.asString }.toTypedArray()))
            }

            DragLine.commandManager.registerCommand(Shortcut(name, script))
        }
    }
    ...
    override fun saveConfig() {
        ...
        for (command in DragLine.commandManager.commands) {

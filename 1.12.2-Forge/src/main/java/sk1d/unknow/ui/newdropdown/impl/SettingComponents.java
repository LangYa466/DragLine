package sk1d.unknow.ui.newdropdown.impl;


import dragline.value.*;
import dragline.utils.render.RenderUtils;
import me.liying.utils.render.LiYingUtil;
...
import sk1d.unknow.ui.newdropdown.utils.animations.impl.EaseInOutQuad;
import sk1d.unknow.ui.newdropdown.utils.normal.Main;
import sk1d.unknow.ui.newdropdown.utils.objects.PasswordField;
import sk1d.unknow.ui.newdropdown.utils.render.DrRenderUtils;
import sk1d.unknow.ui.newdropdown.utils.render.GuiEvents;
...
public class SettingComponents extends Component {
    ...
    public void handle(int mouseX, int mouseY, int button, GuiEvents type) {
        HUD hud = (HUD) DragLine.moduleManager.getModule(HUD.class);
        ...
        Color color2 = new Color(ClickGUI.generateRGB());
        ...
        for (Value setting : module.getValues()) {
            ...
            if (setting instanceof FloatValue) {
                ...
                RenderUtils.drawRound(x + 5, sliderY, totalSliderWidth, 3, 1.5f, LiYingUtil.applyOpacity(darkRectHover, (float) (.4f + (.2 * hoverAnimation.getOutput()))));
                RenderUtils.drawRound(x + 5, sliderY, Math.max(4, sliderfloatMap.get(numberSetting)), 3, 1.5f, accent ? accentedColor2 : textColor);

                LiYingUtil.setAlphaLimit(0.0F);
                LiYingUtil.fakeCircleGlow(x + 4 + Math.max(4, sliderfloatMap.get(numberSetting)), sliderY + 1.5f, 6,
                        Color.BLACK, .3f);

                LiYingUtil.drawGoodCircle(x + 4 + Math.max(4, sliderfloatMap.get(numberSetting)), sliderY + 1.5f, 3.75f, accent ? accentedColor2.getRGB() : textColor.getRGB());
                count += .5f;
            }
            if (setting instanceof IntegerValue) {
                ...
                float sliderY = (settingY + 18);
                RenderUtils.drawRound(x + 5, sliderY, totalSliderWidth, 3, 1.5f, LiYingUtil.applyOpacity(darkRectHover, (float) (.4f + (.2 * hoverAnimation.getOutput()))));
                RenderUtils.drawRound(x + 5, sliderY, Math.max(4, sliderintMap.get(numberSetting)), 3, 1.5f, accent ? accentedColor2 : textColor);

                LiYingUtil.setAlphaLimit(0);
                LiYingUtil.fakeCircleGlow(x + 4 + Math.max(4, sliderintMap.get(numberSetting)), sliderY + 1.5f, 6,
                        Color.BLACK, .3f);

                LiYingUtil.drawGoodCircle(x + 4 + Math.max(4, sliderintMap.get(numberSetting)), sliderY + 1.5f, 3.75f, accent ? accentedColor2.getRGB() : textColor.getRGB());
                count += .5f;
            }
            if (setting instanceof BoolValue) {
                ...
                RenderUtils.drawRound(x + width - (switchWidth + 5.5f), settingY + Fonts.SF.SF_18.SF_18.getMiddleOfBox(rectHeight) + 2, switchWidth, 4.5f,
                ...
                RenderUtils.drawRound((float) (x + width - (switchWidth + 6) + ((switchWidth - 5) * toggleAnimation.getOutput())),
            ...
            if (setting instanceof ListValue) {
                ...
                RenderUtils.drawRound(x + 5, (float) (settingY + rectHeight + 2 + (12 * openAnimation.getOutput())),
                ...
                for (String mode : modeSetting.getValues()) {
                    ...
                    if (modeHoverAnimation.finished(Direction.FORWARDS) || !modeHoverAnimation.isDone()) {
                        RenderUtils.drawRound(x + 5, modeY - 5, width - 10, rectHeight, 3,
                    ...
                    if (openAnimation.isDone() && openAnimation.getDirection().equals(Direction.FORWARDS) || !openAnimation.isDone()) {
                        //      RenderUtils.drawRound(x + 5 + ((width - 10) / 2f - selectRectWidth / 2f), settingY + rectHeight + 10.5f,
                ...
                RenderUtils.drawRound(x + 5, settingY + 5, width - 10, rectHeight + 7, 3, DrRenderUtils.applyOpacity(darkRectHover, .45f));

                if(!hoverAnimation.isDone() || hoverAnimation.finished(Direction.FORWARDS)) {
                    RenderUtils.drawRound(x + 5, settingY + 5, width - 10, rectHeight + 7, 3, DrRenderUtils.applyOpacity(textColor, (float) (.2f * hoverAnimation.getOutput())));
                }
            ...
            /*
                ...
                RenderUtils.drawRound((float) (x + width - (Fonts.SFBOLD.SFBOLD_18.SFBOLD_18.stringWidth(bind) + 9)),

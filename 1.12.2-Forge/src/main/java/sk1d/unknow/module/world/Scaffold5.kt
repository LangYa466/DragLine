...
import dragline.utils.block.BlockUtils.canBeClicked
import dragline.utils.block.BlockUtils.getBlock
import dragline.utils.block.BlockUtils.isReplaceable
import dragline.utils.block.PlaceInfo
import dragline.utils.block.PlaceInfo.Companion.get
...
import me.rainyfall.utils.InventoryUtils.Companion.findAutoBlockBlock
...
@ModuleInfo(name = "Scaffold5", description = "Nel1y", category = ModuleCategory.WORLD)
class Scaffold5 : Module() {
    /**
     * OPTIONS (Scaffold)
     */
    // Mode
    ...
    private val stayAutoBlock = BoolValue("StayAutoBlock", false).displayable { !autoBlockMode.get().equals("Off") }
    ...
    // Basic stuff
    private val settlePitch = BoolValue("GrimAC", true).displayable { rotationsValue.get() }
    private val rotationModeValue = ListValue(
        "RotationMode",
        arrayOf("Hypixel","GrimSprint","Normal", "AAC", "Static", "Static2", "Static3", "Custom"),
        "Normal"
    ).displayable { rotationsValue.get() }  // searching reason
    private val rotationLookupValue = ListValue("RotationLookup", arrayOf("Normal", "AAC", "Same"), "Normal").displayable { rotationsValue.get() }

    private val noHitCheckValue = BoolValue("NoHitCheck", false)
    private val HypixelYawValue = IntegerValue("HypixelYaw", 180, -360, 360).displayable { rotationModeValue.get().equals("Hypixel") }
    private val HypixelPitchValue = IntegerValue("HypixelPitch", 79, 60, 100).displayable { rotationModeValue.get().equals("Hypixel") }
    private val staticPitchValue = FloatValue("Static-Pitch", 86f, 80f, 90f).displayable { rotationModeValue.get().equals("Custom") }
    ...
    }.displayable { rotationModeValue.get().contains("Static") } as FloatValue
    private val customPitchValue = FloatValue("Custom-Pitch", 86f, -90f, 90f).displayable { rotationModeValue.get().contains("Static") }
    private val keepRotationValue = BoolValue("KeepRotation", false)
    private val keepRotOnJumpValue = BoolValue("KeepRotOnJump", true).displayable { keepRotationValue.get() }
    private val keepLengthValue = IntegerValue("KeepRotationLength", 0, 0, 20).displayable { keepRotationValue.get() }
    ...
    private val towerPlaceModeValue = ListValue("Tower-PlaceTiming", arrayOf("Pre", "Post"), "Post").displayable { towerEnabled.get() }
    ...
    // Jump mode
    private val jumpMotionValue = FloatValue("JumpMotion", 0.42f, 0.3681289f, 0.79f).displayable { towerEnabled.get() }
    private val jumpDelayValue = IntegerValue("JumpDelay", 0, 0, 20).displayable { towerEnabled.get() }

    // ConstantMotion
    private val constantMotionValue = FloatValue("ConstantMotion", 0.42f, 0.1f, 1f).displayable { towerEnabled.get() }
    private val constantMotionJumpGroundValue = FloatValue("ConstantMotionJumpGround", 0.79f, 0.76f, 1f).displayable { towerEnabled.get() }

    // Teleport
    private val teleportHeightValue = FloatValue("TeleportHeight", 1.15f, 0.1f, 5f).displayable { towerEnabled.get() }
    private val teleportDelayValue = IntegerValue("TeleportDelay", 0, 0, 20).displayable { towerEnabled.get() }
    private val teleportGroundValue = BoolValue("TeleportGround", true).displayable { towerEnabled.get() }
    private val teleportNoMotionValue = BoolValue("TeleportNoMotion", false).displayable { towerEnabled.get() }

    // Zitter
    private val zitterValue = BoolValue("Zitter", false)
    private val zitterModeValue = ListValue("ZitterMode", arrayOf("Teleport", "Smooth"), "Teleport").displayable { zitterValue.get() }
    ...
    private val zitterDelay = IntegerValue("ZitterDelay", 100, 0, 500).displayable { zitterValue.get() }
    ...
    override fun onEnable() {
        ...
        if (autoDisableSpeedValue.get() && DragLine.moduleManager.getModule(Speed::class.java).state) {
            DragLine.moduleManager.getModule(Speed::class.java).state = false
        }
        faceBlock = false
    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        if (shouldGoDown) {
            ...
            if (!autoJumpValue.get() && !(smartSpeedValue.get() && DragLine.moduleManager.getModule(Speed::class.java).state) || mc.gameSettings.isKeyDown(
                    mc.gameSettings.keyBindJump
                ) || mc.thePlayer!!.posY < launchY
            ) launchY = mc.thePlayer!!.posY.toInt()
            if (autoJumpValue.get() && !DragLine.moduleManager.getModule(Speed::class.java).state && isMoving && mc.thePlayer!!.onGround) {
                mc.thePlayer!!.jump()
            }
        }
    }
    ...
    fun onMotion(event: MotionEvent) {
        ...
        if(rotationModeValue.get().equals("GrimSprint") ) {
           RotationUtils.serverRotation.pitch = DragLine.getPitch()
        }
    ...
    private fun findBlock(expand: Boolean) {
        val WBlockPosition = if (shouldGoDown) (if (mc.thePlayer!!.posY == mc.thePlayer!!.posY.toInt() + 0.5) WBlockPos(
            ...
            mc.thePlayer!!.posX, mc.thePlayer!!.posY - 0.6, mc.thePlayer!!.posZ
        ).down()) else if (!towerActivation() && (sameYValue.get() || (autoJumpValue.get() || smartSpeedValue.get() && DragLine.moduleManager.getModule(
                Speed::class.java
    ...
    private fun place(towerActive: Boolean) {
        ...
        if (!towerActivation() && (!delayTimer.hasTimePassed(delay) && (sameYValue.get() || (autoJumpValue.get() || smartSpeedValue.get() && DragLine.moduleManager.getModule(
                Speed::class.java
    ...
    fun onRender3D(event: Render3DEvent?) {
        ...
        for (i in 0 until if (modeValue.get()
            ...
            val wBlockPos = WBlockPos(
                ...
                if (!towerActivation()
                    && (sameYValue.get() ||
                            ((autoJumpValue.get() || smartSpeedValue.get() && DragLine.moduleManager.getModule(Speed::class.java).state)

...
class Scaffold4 : Module() {
    ...
    // Delay
    private val maxDelayValue: IntegerValue = object : IntegerValue("Max Delay", 30, 0, 1000) {
    ...
    private val minDelayValue: IntegerValue = object : IntegerValue("Min Delay", 50, 0, 1000) {
        override fun onChanged(oldValue: Int, newValue: Int) {
    ...
    private val placeModeValue = ListValue("Place Timing", arrayOf("Pre", "Post", "Tick"), "Tick")
    ...
    private val slowSpeed = FloatValue("Slow Speed", 0.83f, 0.2f, 1.0f)

    // Safety
    private val sameYValue = BoolValue("Same Y", true)
    ...
    fun onRender2D(event: Render2DEvent) {
        if (counterDisplayValue.get()) {
            GL11.glPushMatrix()
            val blockOverlay = DragLine.moduleManager.getModule(BlockOverlay::class.java) as BlockOverlay
